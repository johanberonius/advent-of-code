...
while (@q) {
    ...
    if ($x == $ex && $y == $ey) {
        if (!defined $m || $s <= $m) {
            $m = $s;
            print "Found new one of best paths: $m\n";
            $p{$_}++ for @$p;
        }
        next;
    }
    ...
    my ($nx, $ny) = ($x+$dx, $y+$dy);
    unshift @q => [$nx, $ny, $dx, $dy, $s+1, [@$p, "$nx,$ny"]] if $g{"$nx,$ny"} ne '#';
    push @q => [$x, $y, $dy, -$dx, $s+1000, $p];
    push @q => [$x, $y, -$dy, $dx, $s+1000, $p];
...
print "Iterations: $i\n";
print "Min score: $m\n";
print "Tiles part of any best path: ", 0+keys %p, "\n";
