...
while (@q) {
    ...
    unshift @q => [$x, $sy, $s, {%$v}, $tx || $x, $ty || $sy, $ts] if $gs eq '.' && !$vs;
    unless ($x == 19 && $y == 19) { # test1, only go south to exit.
        unshift @q => [$ex, $y, $s, {%$v}, $tx || $ex, $ty || $y, $ts] if $ge eq '.' && !$ve;
        unshift @q => [$x, $ny, $s, {%$v}, $tx || $x, $ty || $ny, $ts] if $gn eq '.' && !$vn;
        unshift @q => [$wx, $y, $s, {%$v}, $tx || $wx, $ty || $y, $ts] if $gw eq '.' && !$vw;
    }
