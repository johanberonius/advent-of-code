...
while (@p) {
    ...
    # Stop if path is longer than minimum found for this position and combination of keys
    my $k = join ',', $x, $y, sort keys %k;
    if ($dmin{$k} && $d >= $dmin{$k}) {
        next;
    }
...
# 4762 is the right answer, no branch limit, ~27 K iterations
