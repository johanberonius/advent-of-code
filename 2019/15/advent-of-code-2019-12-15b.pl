...
L: while (@q) {
    ...
    for my $dir (@$q) {
        ...
        if ($o == 2) {
            ...
            # Backtrack
            for my $dir (reverse @$q) {
                run($rev{$dir}) == 1 or die "Unsuccessfull backtrack move after oxygen square";
            }
            ($x, $y) = (0, 0);
        }
    }
...
for my $py (min(@y) .. max(@y)) {
    for my $px (min(@x) .. max(@x)) {
        my $b = $c{"$px,$py"};
        print MAP $b if $b eq '#' || $b eq 'O';
        print MAP '.' if $b > 0 || $b eq '.';
        print MAP '#' unless $b;
    }
    print MAP "\n";
}
...
sub draw {
    ...
    for my $py (min(@y, -21) .. max(@y, 21)) {
        for my $px (min(@x, -21) .. max(@x, 21)) {
            my $b = $c{"$px,$py"};
            print color('on_black'), '   ' if  $b eq '#';
            print color('on_blue'), ' O ' if  $b eq 'O';
            print color('on_green'), sprintf '%3d', $b if $b > 0 || $b eq '.';
